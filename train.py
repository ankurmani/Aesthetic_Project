...
from util import apply_transformations_tf, supervised_loss_fn, brisque_loss_tf, build_nima_mobilenet_model, earth_mover_distance
...
# Dataset
batch_size = 3  # Use higher batch size for multi-GPU
epochs = 5
...
nima_model = build_nima_mobilenet_model()
...
for epoch in range(epochs):
    ...
    for step, batch in enumerate(dataset):
        ...
        with tf.GradientTape() as tape:
            ...
            #brisque_loss = brisque_loss_tf(transformed)
            nima_pred = nima_model(transformed)
            nima_scores = earth_mover_distance(nima_pred)
            nima_loss = tf.reduce_mean(nima_scores)
            aesth_loss = nima_loss #+ 0.2 * brisque_loss
            comp_loss = tf.keras.losses.categorical_crossentropy(y_class, classify)
            comp_loss = tf.reduce_mean(comp_loss)
